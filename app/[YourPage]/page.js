...
function Page() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (data.res) {
        setData(data.res);
        const result = await fetch(`/api/paymentGet?userName=${data.res.userName}` , {method : "GET" })
         
        const dataP=await result.json()
        console.log(dataP.error?dataP.error:dataP.res)
        if(dataP.res) { 
 ...
 console.log(payment)
    ...
    <>
      {onLoading ? (
        ...
        >
          ...
          {/* User Info */}
          <motion.div
            className="min-h-[31vh] sm:min-h-[30vh] w-[100vw] flex justify-center items-start mt-3"
            variants={fadeInUp}
            
          >
            <div className="flex flex-col justify-center items-center mx-0 w-[100vw] pt-[7vh] gap-3">
              <motion.div
          ...
          >
            ...
            <motion.div
              className="make-payments w-[80%] h-[40%] lg:w-[40%] lg:h-[90%] bg-gradient-to-br from-indigo-400 via-indigo-500 to-indigo-600 rounded-3xl shadow-[0px_0px_10px_rgba(72,125,239,0.3)] text-[#001427] font-extrabold text-2xl pt-7 flex flex-col justify-center items-center gap-5 cursor-pointer hover:scale-102 transition-all duration-200 ease-in-out"
              variants={fadeInUp}
              
            >
              <span className="pr-[40%]">Supporters</span>
  ...
  <thead className="border-b border-white/30 text-left">
    <tr className="flex justify-between px-5">
      <th className="w-[33%] text-lgtext-sm ">Name</th>
      <th className="w-[33%] text-center text-lgtext-sm ">Amount</th>
      <th className="w-[33%] text-right text-lgtext-sm ">Comment</th>
    </tr>
  </thead>
  <tbody className="flex flex-col gap-2 overflow-y-auto max-h-[30vh]">
    {payment && payment.length > 0 ? (
      payment.map((i, idx) => (
        ...
        >
          <td className="text-sm w-[33%] text-left">{i.fuserName}</td>
          <td className="text-sm w-[33%] text-center">${i.amount}</td>
          <td className="text-sm w-[33%] text-right italic text-gray-300">
            {i.comments || "—"}
          </td>
        </tr>
      ))
      ...
      <tr className="flex justify-center items-center py-4">
        <td className="text-center text-gray-400">No Supporters yet</td>
      </tr>
    )}
  </tbody>
</table>
